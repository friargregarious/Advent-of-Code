...
def around_me(my_pos: tuple, garden: dict):
    """
    I made this function in 2024 day 10, probably for the 10th time. Now I'm adding it to my little list of usful functions.
    Return a list of valid (N, S, E, W) directions a climber can move from the given position in the topological map.

    :param my_pos: tuple of (row, col) specifying the position in the topological map
    :param topo: a dictionary representing the topological map. Each key is a tuple of (row, col) and the value is the height at that position
    :return: a list of valid direction tuples that the climber can move from the given position
    """
    MAX_ROW = 0
    MAX_COL = 0
    for r, c in garden.keys():
        MAX_ROW = r if r > MAX_ROW else MAX_ROW
        MAX_COL = c if c > MAX_COL else MAX_COL
